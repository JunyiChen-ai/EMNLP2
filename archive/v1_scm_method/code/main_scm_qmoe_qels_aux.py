"""
SCM + Q-MoE + QELS + SCM Auxiliary Tasks.

Round 008 Step 3: Add two SCM-grounded auxiliary heads:
1. Quadrant classification: predict which SCM quadrant (4-way) using pseudo-labels from LLM
2. Warmth polarity: predict warm vs cold from warmth_evidence

These auxiliary tasks improve feature quality by enforcing SCM structure in the learned representation.

Literature:
- Plaza-del-Arco et al. (2024) "Generalizing Hate Speech Detection Using Multi-Task Learning"
- HAMiSoN-MTL (CASE @ ACL 2024)
- Cuddy et al. (2007) BIAS Map

Usage:
  python main_scm_qmoe_qels_aux.py --dataset_name HateMM --num_runs 200 --seed_offset 0
"""
...
# Quadrant pseudo-labels from social_perception field
QUADRANT_MAP = {
    "contempt": 0, "envy": 1, "pity": 2, "admiration": 3,
    "mixed": -1, "none": -1, "": -1
}


class DSAux(Dataset):
    """Dataset with auxiliary labels (quadrant pseudo-label)."""
    def __init__(self, vids, feats, lm, mk, scm_data_map):
        self.vids = vids; self.f = feats; self.lm = lm; self.mk = mk
        self.scm_map = scm_data_map
    ...
    def __getitem__(self, i):
        ...
        # Quadrant pseudo-label from LLM's social_perception field
        scm_resp = self.scm_map.get(v, {})
        sp = scm_resp.get("social_perception", "").lower().strip()
        # Extract first word matching a quadrant
        quad = -1
        for key, val in QUADRANT_MAP.items():
            if key and key in sp:
                quad = val; break
        out["quadrant_label"] = torch.tensor(quad, dtype=torch.long)
        return out
...
class SCMQMoEQELSAux(nn.Module):
    """SCM + Q-MoE + QELS + Auxiliary Heads."""
    def __init__(self, base_mk, hidden=192, nh=4, nc=2, drop=0.15, md=0.15,
        ...
        # Auxiliary head 1: quadrant classification (4-way)
        self.aux_quadrant_head = nn.Sequential(
            nn.Linear(hidden * 2, hidden // 2), nn.GELU(),
            nn.Linear(hidden // 2, n_quadrants)
        )

    def forward(self, batch, training=False, return_penult=False, return_qels=False, return_aux=False):
        base_feats = []
        ...
        if return_aux:
            # Auxiliary: quadrant classification from wc_cat
            aux_quad_logits = self.aux_quadrant_head(wc_cat)
            q_entropy = -(quadrant_dist * (quadrant_dist + 1e-8).log()).sum(dim=-1)
            q_entropy_norm = q_entropy / np.log(self.n_quadrants)
            return logits, q_entropy_norm, aux_quad_logits
...
def setup_logger(tag=""):
    ...
    lf = f"./logs/scm_qmoe_qels_aux_{tag}_{ts}.log"
    logger = logging.getLogger(f"aux_{tag}_{ts}")
...
def run(feats, splits, lm, base_mk, nc, num_runs, seed_offset, class_weight, save_dir, logger,
        scm_data_map, expert_hidden=64, eps_min=0.01, eps_lambda=0.15, aux_weight=0.3):
    ...
    aux_crit = nn.CrossEntropyLoss(ignore_index=-1)  # ignore unlabeled quadrants
    ...
    for ri in range(num_runs):
        ...
        trl = DataLoader(DSAux(cur["train"], feats, lm, all_mk, scm_data_map), 32, True, collate_fn=collate_fn)
        vl = DataLoader(DSAux(cur["valid"], feats, lm, all_mk, scm_data_map), 64, False, collate_fn=collate_fn)
        tel = DataLoader(DSAux(cur["test"], feats, lm, all_mk, scm_data_map), 64, False, collate_fn=collate_fn)

        model = SCMQMoEQELSAux(base_mk, nc=nc, expert_hidden=expert_hidden,
        ...
        for e in range(ep):
            ...
            for batch in trl:
                ...
                logits, q_entropy, aux_quad_logits = model(batch, training=True, return_aux=True)
                # Main loss
                ...
                # Auxiliary quadrant loss (only on samples with valid pseudo-labels)
                quad_loss = aux_crit(aux_quad_logits, batch["quadrant_label"])
                loss = main_loss + aux_weight * quad_loss
                loss.backward()
        ...
        head_acc, _ = best_thresh(vl_logits, vl_lab, te_logits, te_lab)

        if _ is not None:
            preds = ((te_logits[:, 1] - te_logits[:, 0]) > _).astype(int)
        else:
...
def main():
    ...
    if args.dataset_name == "HateMM":
        ...
        scm_data_path = "./datasets/HateMM/scm_data.json"
    else:
        ...
        scm_data_path = f"./datasets/Multihateclip/{args.language}/scm_data.json"

    logger = setup_logger(f"AUX_{tag}")
    nc = 2; base_mk = ["text", "audio", "frame"]
    logger.info(f"=== SCM Q-MoE+QELS+AUX: {tag}, Runs: {args.num_runs}, aux_weight={args.aux_weight} ===")
    ...
    # Load SCM rationale data for pseudo-labels
    with open(scm_data_path) as f:
        scm_data = json.load(f)
    scm_data_map = {d["Video_ID"]: d.get("scm_response", {}) for d in scm_data}

    # Count quadrant pseudo-labels
    quad_counts = {}
    for vid, resp in scm_data_map.items():
        sp = resp.get("social_perception", "").lower().strip()
        for key in QUADRANT_MAP:
            if key and key in sp:
                quad_counts[key] = quad_counts.get(key, 0) + 1; break
    logger.info(f"  Quadrant pseudo-label distribution: {quad_counts}")

    splits = load_split_ids(split_dir)
    save_dir = f"./seed_search_scm_qmoe_qels_aux/{tag}_off{args.seed_offset}_aw{args.aux_weight}"

    best = run(feats, splits, lm, base_mk, nc, args.num_runs, args.seed_offset,
               class_weight=[1.0, 1.5], save_dir=save_dir, logger=logger,
               scm_data_map=scm_data_map, aux_weight=args.aux_weight)

    logger.info(f"\n{'='*60}")
    logger.info(f"  AUX {tag} BEST: ACC={best['metrics']['acc']:.4f} F1={best['metrics']['f1']:.4f}")
    logger.info(f"{'='*60}")
