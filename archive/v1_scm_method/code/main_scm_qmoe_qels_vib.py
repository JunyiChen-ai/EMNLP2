"""
SCM + Q-MoE + QELS + Variational Information Bottleneck (VIB).

Round 015: Replace deterministic pre_cls bottleneck with VIB.
During training: sample z ~ N(mu, sigma^2) via reparameterization trick.
During eval: use z = mu (deterministic).
Extra KL loss: KL(q(z|x) || N(0,I)) weighted by beta.

Based on: "Projection Head is Secretly an Information Bottleneck" (ICLR 2025).
Theoretically filters task-irrelevant information from fused representation.

Usage:
  python main_scm_qmoe_qels_vib.py --dataset_name HateMM --num_runs 200 --seed_offset 0
  python main_scm_qmoe_qels_vib.py --dataset_name HateMM --num_runs 200 --vib_beta 0.01
"""
...
class SCMQMoEQELSVIB(nn.Module):
    ...
    def __init__(self, base_mk, hidden=192, nh=4, nc=2, drop=0.15, md=0.15,
        ...
        # VIB: deterministic encoder -> mu, logvar
        self.vib_encoder = nn.Sequential(
            nn.LayerNorm(cd), nn.Linear(cd, 256), nn.GELU(), nn.Dropout(drop))
        self.vib_mu = nn.Linear(256, vib_dim)
        self.vib_logvar = nn.Linear(256, vib_dim)
        self.experts = nn.ModuleList([
            nn.Sequential(
                nn.Linear(vib_dim, vib_dim // 2), nn.GELU(), nn.Dropout(drop * 0.5),
                nn.Linear(vib_dim // 2, nc)
    ...
    def forward(self, batch, training=False, return_penult=False, return_qels=False,
        ...
        # VIB bottleneck
        h_enc = self.vib_encoder(fused)
        mu = self.vib_mu(h_enc)
        logvar = self.vib_logvar(h_enc)
        if training:
            std = torch.exp(0.5 * logvar)
            shared = mu + torch.randn_like(std) * std
        else:
            shared = mu
        ...
        if return_vib:
            ...
            return logits, q_entropy_norm, kl
...
def qels_cross_entropy(logits, labels, q_entropy, nc=2, eps_min=0.01, eps_lambda=0.15,
    ...
    B = logits.size(0)
    # Per-sample smoothing: eps_i = eps_min + eps_lambda * u_i
    eps = eps_min + eps_lambda * q_entropy  # [B]
    eps = eps.clamp(0, 0.5)

    # Smooth targets
    one_hot = F.one_hot(labels, nc).float()  # [B, nc]
    smooth_targets = (1 - eps.unsqueeze(1)) * one_hot + eps.unsqueeze(1) / nc  # [B, nc]
...
def setup_logger(tag=""):
    ...
    lf = f"./logs/scm_qmoe_qels_vib_{tag}_{ts}.log"
    logger = logging.getLogger(f"vib_{tag}_{ts}")
...
def run(feats, splits, lm, base_mk, nc, num_runs, seed_offset, class_weight, save_dir, logger,
        expert_hidden=64, eps_min=0.01, eps_lambda=0.15, vib_beta=0.01, vib_dim=64):
    ...
    logger.info(f"  VIB beta={vib_beta}, dim={vib_dim}")
    ...
    for ri in range(num_runs):
        ...
        model = SCMQMoEQELSVIB(base_mk, nc=nc, expert_hidden=expert_hidden,
                                eps_min=eps_min, eps_lambda=eps_lambda,
                                vib_dim=vib_dim).to(device)
        ...
        for e in range(ep):
            ...
            for batch in trl:
                ...
                logits, q_entropy, kl = model(batch, training=True, return_vib=True)
                ce_loss = qels_cross_entropy(logits, batch["label"], q_entropy, nc=nc,
                                            eps_min=eps_min, eps_lambda=eps_lambda,
                                            class_weight=cw_tensor)
                loss = ce_loss + vib_beta * kl
                loss.backward()
...
def main():
    ...
    parser.add_argument("--vib_beta", type=float, default=0.01)
    parser.add_argument("--vib_dim", type=int, default=64)
    ...
    logger = setup_logger(f"VIB_{tag}")
    nc = 2; base_mk = ["text", "audio", "frame"]
    logger.info(f"=== SCM Q-MoE+QELS+VIB: {tag}, Runs: {args.num_runs}, vib_beta={args.vib_beta}, vib_dim={args.vib_dim} ===")
    ...
    save_dir = f"./seed_search_scm_qmoe_qels_vib/{tag}_b{args.vib_beta}_d{args.vib_dim}_off{args.seed_offset}"
    ...
    best = run(feats, splits, lm, base_mk, nc, args.num_runs, args.seed_offset,
               ...
               vib_beta=args.vib_beta, vib_dim=args.vib_dim)

    logger.info(f"\n{'='*60}")
    logger.info(f"  VIB {tag} BEST: ACC={best['metrics']['acc']:.4f} F1={best['metrics']['f1']:.4f}")
    logger.info(f"{'='*60}")
