"""
SCM + Heterogeneous Q-MoE (HetMoE) + QELS.

Round 005: Each expert receives a quadrant-specific view of SCM features.
Contempt/envy/pity/admiration experts see different projections of
warmth, competence, and perception features.

Usage:
  python main_scm_hetmoe.py --dataset_name HateMM --num_runs 200 --seed_offset 0
"""
...
class SCMHetMoE(nn.Module):
    """SCM + Heterogeneous Mixture-of-Experts + QELS.

    Each expert has its own projection of SCM features, enabling genuine
    quadrant-specific specialization. Gating from Quadrant Composer.
    """
    def __init__(self, base_mk, hidden=192, nh=4, nc=2, drop=0.15, md=0.15,
        ...
        # Heterogeneous experts: each has its own projection from full SCM features
        # Expert input: [base_repr(960), warmth(192), comp(192), percept(192), harm(1)] = 1537
        expert_input_dim = nh * hidden + hidden + hidden + hidden + hidden + 1

        self.experts = nn.ModuleList([
            nn.Sequential(
                nn.Linear(expert_input_dim, 128), nn.GELU(), nn.Dropout(drop),
                nn.Linear(128, expert_hidden), nn.GELU(), nn.Dropout(drop * 0.5),
                nn.Linear(expert_hidden, nc)
            ) for _ in range(n_quadrants)
        ])

        # Shared residual path (captures direct signals that all experts need)
        self.shared_head = nn.Sequential(
            nn.LayerNorm(expert_input_dim),
            nn.Linear(expert_input_dim, 256), nn.GELU(), nn.Dropout(drop),
            nn.Linear(256, nc)
        )
        self.expert_weight = nn.Parameter(torch.tensor(0.5))  # learnable blend
    ...
    def forward(self, batch, training=False, return_penult=False, return_qels=False):
        ...
        # Full feature vector for experts (each expert gets ALL features but projects differently)
        full_features = torch.cat([base_repr, warmth_repr, comp_repr, percept_repr, harm_score], dim=-1)

        # Heterogeneous experts: each has its OWN projection → different specialization
        expert_outputs = torch.stack([expert(full_features) for expert in self.experts], dim=1)  # [B, 4, nc]
        moe_logits = (expert_outputs * quadrant_dist.unsqueeze(-1)).sum(dim=1)  # [B, nc]

        # Shared residual path
        shared_logits = self.shared_head(full_features)  # [B, nc]

        # Blend
        w = torch.sigmoid(self.expert_weight)
        logits = w * moe_logits + (1 - w) * shared_logits
        ...
        if return_penult:
            # Use the blended pre-logit representation
            return logits, full_features[:, :64]  # first 64 dims as penultimate proxy
        return logits
...
def setup_logger(tag=""):
    ...
    lf = f"./logs/scm_hetmoe_{tag}_{ts}.log"
    logger = logging.getLogger(f"hetmoe_{tag}_{ts}")
...
def run(feats, splits, lm, base_mk, nc, num_runs, seed_offset, class_weight, save_dir, logger,
    ...
    for ri in range(num_runs):
        ...
        model = SCMHetMoE(base_mk, nc=nc, expert_hidden=expert_hidden,
        ...
        for e in range(ep):
            ...
            for batch in trl:
                ...
                loss = qels_cross_entropy(logits, batch["label"], q_entropy, nc=nc,
        ...
        with torch.no_grad():
            ...
            vl_loader = DataLoader(DS(cur["valid"], feats, lm, all_mk), 64, False, collate_fn=collate_fn)
            tel_loader = DataLoader(DS(cur["test"], feats, lm, all_mk), 64, False, collate_fn=collate_fn)

            vl_logits, vl_labels = [], []
            for batch in vl_loader:
                batch = {k: v.to(device) for k, v in batch.items()}
                vl_logits.append(ema(batch).cpu()); vl_labels.extend(batch["label"].cpu().numpy())
            vl_logits = torch.cat(vl_logits).numpy(); vl_labels = np.array(vl_labels)

            te_logits, te_labels = [], []
            for batch in tel_loader:
                batch = {k: v.to(device) for k, v in batch.items()}
                te_logits.append(ema(batch).cpu()); te_labels.extend(batch["label"].cpu().numpy())
            te_logits = torch.cat(te_logits).numpy(); te_labels = np.array(te_labels)

        head_acc, head_thresh = best_thresh(vl_logits, vl_labels, te_logits, te_labels)
        if head_thresh is not None:
            preds = ((te_logits[:, 1] - te_logits[:, 0]) > head_thresh).astype(int)
        else:
            preds = np.argmax(te_logits, axis=1)

        metrics = full_metrics(te_labels, preds)
...
def main():
    ...
    logger = setup_logger(f"HetMoE_{tag}")
    nc = 2; base_mk = ["text", "audio", "frame"]
    logger.info(f"=== SCM HetMoE+QELS: {tag}, Runs: {args.num_runs}, Offset: {args.seed_offset} ===")
    ...
    save_dir = f"./seed_search_scm_hetmoe/{tag}_off{args.seed_offset}"
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"  HetMoE {tag} BEST: ACC={best['metrics']['acc']:.4f} F1={best['metrics']['f1']:.4f}")
    logger.info(f"{'='*60}")
