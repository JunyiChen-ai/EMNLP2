"""
Ablation study for ITT model.
Runs on the best seed/config for each dataset.
Each ablation modifies one aspect of the model.

Usage:
  conda run -n HVGuard python ablation_itt.py --dataset_name HateMM
  conda run -n HVGuard python ablation_itt.py --dataset_name Multihateclip --language English
  conda run -n HVGuard python ablation_itt.py --dataset_name Multihateclip --language Chinese
"""
...
# ---- Full ITT model (copied from main_itt.py, not importing to keep independent) ----
class ITTFusion(nn.Module):
    def __init__(self, base_mk, hidden=192, nh=4, nc=2, drop=0.15, md=0.15,
                 disable_channels=None, disable_moderation=False, collapse_channels=False,
                 disable_fields=None):
        super().__init__()
        self.base_mk = base_mk; self.md = md; self.hidden = hidden
        self.disable_channels = disable_channels or []
        self.disable_moderation = disable_moderation
        ...
        if collapse_channels:
            # Single channel for all threat fields combined
            self.single_channel = nn.Sequential(
                nn.Linear(hidden * 5, hidden), nn.GELU(), nn.Dropout(drop),
        ...
        active_channels = 4 - len(self.disable_channels)
        if collapse_channels:
            self.channel_gate = None
            self.accum_proj = nn.Sequential(
                nn.Linear(hidden, hidden * 2), nn.GELU(), nn.Dropout(drop), nn.LayerNorm(hidden * 2)
            )
        else:
            self.channel_gate = nn.Sequential(nn.Linear(hidden * active_channels, active_channels))
            self.accum_proj = nn.Sequential(
                nn.Linear(hidden * active_channels, hidden * 2), nn.GELU(), nn.Dropout(drop), nn.LayerNorm(hidden * 2)
            )
        ...
        cd = nh * hidden + hidden + hidden * 2 + (0 if disable_moderation else 1)
    ...
    def forward(self, batch, training=False, return_penult=False):
        ...
        if self.collapse_channels:
            all_threat = torch.cat([field_h[CHANNEL_FIELDS[ch]] for ch in CHANNEL_FIELDS]
                                   + [base_ctx], dim=-1)
            threat_out = self.single_channel(all_threat)
            threat_repr = self.accum_proj(threat_out)
        else:
            channel_outputs = []
            for ch_name, ch_field in CHANNEL_FIELDS.items():
                if ch_name in self.disable_channels:
                    continue
                ch_input = torch.cat([field_h[ch_field], base_ctx], dim=-1)
                ch_out = self.channel_attn[ch_name](ch_input)
                channel_outputs.append(ch_out)

            ch_stack = torch.stack(channel_outputs, dim=1)
            ch_cat = torch.cat(channel_outputs, dim=-1)
            ch_weights = torch.softmax(self.channel_gate(ch_cat), dim=-1)
            threat_repr = self.accum_proj(ch_cat)

        if not self.disable_moderation:
            ...
            fused = torch.cat([base_repr, threat_repr], dim=-1)
...
# ---- Baseline: plain pooled rationale (no theory fusion) ----
...
def apply_retrieval(model, feats, splits, lm, mk, nc, best_config, seed):
    """Apply the best retrieval config and return metrics."""
    ...
    tp, tl_arr, tla = get_pl(model, trl_ns)
    vp, vl_arr, vla = get_pl(model, vl)
    tep, tel_arr, tela = get_pl(model, tel)
    blt = torch.tensor(tla)

    wname = best_config.get("whiten", "none")
    if wname == "zca":
        tp, vp, tep = zca_whiten(tp, vp, tep)
    elif wname.startswith("spca_r"):
        r = int(wname.split("r")[1])
        tp, vp, tep = shrinkage_pca_whiten(tp, vp, tep, r=r)
...
def train_model(model, feats, splits, lm, mk, nc, seed, class_weight):
    """Train model and return trained EMA model."""
    torch.manual_seed(seed); random.seed(seed); np.random.seed(seed)
    common = set.intersection(*[set(feats[k].keys()) for k in mk]) & set(feats["labels"].keys())
    cur = {s: [v for v in splits[s] if v in common] for s in splits}
...
def main():
    ...
    # Load best config
    results_path = f"./seed_search_itt/{tag}_off0/all_results.json"
    ...
    logger = logging.getLogger(f"abl_itt_{tag}_{ts}")
    logger.setLevel(logging.INFO); logger.handlers.clear()
    fh = logging.FileHandler(f"./logs/ablation_itt_{tag}_{ts}.log", encoding="utf-8")
    ...
    logger.info(f"=== ITT Ablation Study: {tag} ===")
    logger.info(f"Best seed={seed}, config={best_config}")
    logger.info(f"Best ACC={best['metrics']['acc']:.4f} F1={best['metrics']['f1']:.4f}")

    results = []

    # ABL-00: Full model (reproduce best)
    results.append(run_ablation("ABL-00", "Full ITT model (reproduce)",
        lambda: ITTFusion(base_mk, nc=nc), all_mk, feats, splits, lm, nc, seed, best_config, cw_val, logger))

    # ABL-01: No rationale (text+audio+frame only)
    ...
    # ABL-02: Pooled rationale (no theory-aware fusion)
    pooled_mk = base_mk + ["itt_rationale"]
    ...
    # ABL-03: No Threat Moderation Block
    results.append(run_ablation("ABL-03", "No Threat Moderation Block",
        lambda: ITTFusion(base_mk, nc=nc, disable_moderation=True),
        all_mk, feats, splits, lm, nc, seed, best_config, cw_val, logger))

    # ABL-04: Collapse 4 channels → 1 (no parallel channels)
    results.append(run_ablation("ABL-04", "Single channel (no parallel threat channels)",
        lambda: ITTFusion(base_mk, nc=nc, collapse_channels=True),
        all_mk, feats, splits, lm, nc, seed, best_config, cw_val, logger))

    # ABL-05 to ABL-08: Remove individual channels
    for ch_name in ["realistic", "symbolic", "anxiety", "hostility"]:
        results.append(run_ablation(f"ABL-05-{ch_name}", f"Remove {ch_name} threat channel",
            lambda cn=ch_name: ITTFusion(base_mk, nc=nc, disable_channels=[cn]),
            all_mk, feats, splits, lm, nc, seed, best_config, cw_val, logger))

    # ABL-09 to ABL-14: Remove individual fields
    for field in ITT_FIELDS:
        results.append(run_ablation(f"ABL-06-{field}", f"Remove field: {field}",
            lambda fld=field: ITTFusion(base_mk, nc=nc, disable_fields=[fld]),
            all_mk, feats, splits, lm, nc, seed, best_config, cw_val, logger))

    # Summary
    logger.info(f"\n{'='*70}")
    logger.info(f"ABLATION SUMMARY — ITT {tag}")
    logger.info(f"{'='*70}")
    logger.info(f"{'ID':<25} {'Description':<45} {'ACC':>7} {'F1':>7} {'Δ F1':>7}")
    logger.info(f"{'-'*91}")
    base_f1 = results[0]["f1"]
    for r in results:
        delta = r["f1"] - base_f1
        logger.info(f"{r['abl_id']:<25} {r['desc']:<45} {r['acc']:>7.4f} {r['f1']:>7.4f} {delta:>+7.4f}")

    os.makedirs("./ablation_results", exist_ok=True)
    save_path = f"./ablation_results/itt_{tag}_ablations.json"
