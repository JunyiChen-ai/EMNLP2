"""
SCM + Q-MoE + QELS + MR2-lite (Margin Regularization via Representation).

Round 014: Add class-variance-conditioned logit margin + representation
compactness on the shared (64-dim) penultimate tensor.

Based on: "Reducing Class-Wise Performance Disparity" (MR2), ICLR 2026.
Core idea: margin proportional to per-class feature variability.
  - Logit margin: shift logits by class-dependent margin proportional to feature spread
  - Representation regularization: reduce within-class variance of shared tensor

Also enables load-balance loss on quadrant distribution (was declared but never used).

Usage:
  python main_scm_qmoe_qels_mr2.py --dataset_name HateMM --num_runs 10 --seed_offset 0
  python main_scm_qmoe_qels_mr2.py --dataset_name HateMM --num_runs 10 --mr2_alpha 0.3 --mr2_beta 0.1
"""
...
class SCMQMoEQELSMR2(nn.Module):
    """SCM + Q-MoE + QELS + MR2-lite margin regularization."""
    def __init__(self, base_mk, hidden=192, nh=4, nc=2, drop=0.15, md=0.15,
        ...
        cd = nh * hidden + hidden + hidden + hidden  # base_repr + quadrant_repr + percept_repr
...
def qels_cross_entropy(logits, labels, q_entropy, nc=2, eps_min=0.01, eps_lambda=0.15,
                       class_weight=None):
    """Cross-entropy with Quadrant-Entropy Adaptive Label Smoothing."""
...
def mr2_loss(shared, labels, nc=2):
    """MR2-lite: within-class compactness + between-class separation on penultimate features.

    Based on MR2 (ICLR 2026): margin proportional to per-class feature variability.
    Compactness: minimize within-class variance (reduces feature spread).
    Separation: maximize distance between class centroids.
    """
    # Compute per-class centroids and variance
    centroids = []
    intra_var = []
    for c in range(nc):
        mask = (labels == c)
        ...
        # Within-class variance (trace of covariance)
        var = ((class_feats - centroid.unsqueeze(0)) ** 2).mean()
        intra_var.append(var)

    # Compactness loss: sum of within-class variances
    compact_loss = sum(intra_var) / nc

    # Separation loss: negative distance between centroids (push apart)
    if len(centroids) == 2:
...
def load_balance_loss(quadrant_dist):
    """Encourage uniform usage of all experts.

    Loss = CV^2 of mean expert usage across batch.
    """
    # Mean usage per expert across batch
    usage = quadrant_dist.mean(dim=0)  # [n_quadrants]
...
def setup_logger(tag=""):
    ...
    lf = f"./logs/scm_qmoe_qels_mr2_{tag}_{ts}.log"
    logger = logging.getLogger(f"mr2_{tag}_{ts}")
...
def run(feats, splits, lm, base_mk, nc, num_runs, seed_offset, class_weight, save_dir, logger,
        ...
        mr2_alpha=0.1, mr2_beta=0.05, lb_weight=0.01):
    """
    mr2_alpha: weight for compactness loss (within-class variance reduction)
    mr2_beta: weight for separation loss (between-class centroid push)
    lb_weight: weight for load-balance loss on quadrant distribution
    """
    ...
    logger.info(f"  MR2 alpha={mr2_alpha}, beta={mr2_beta}, lb_weight={lb_weight}")
    ...
    for ri in range(num_runs):
        ...
        model = SCMQMoEQELSMR2(base_mk, nc=nc, expert_hidden=expert_hidden,
                                eps_min=eps_min, eps_lambda=eps_lambda,
                                load_balance_weight=lb_weight).to(device)
        ...
        for e in range(ep):
            ...
            for batch in trl:
                ...
                # 1. QELS cross-entropy (base loss)
                ce_loss = qels_cross_entropy(logits, batch["label"], q_entropy, nc=nc,
                                            eps_min=eps_min, eps_lambda=eps_lambda,
                                            class_weight=cw_tensor)

                # 2. MR2: compactness + separation on shared (penultimate) features
                compact, sep = mr2_loss(shared, batch["label"], nc=nc)

                # 3. Load-balance loss on quadrant distribution
        ...
        result = {"seed": seed, "ri": ri, "seed_offset": seed_offset,
                  ...
                  "mr2_alpha": mr2_alpha, "mr2_beta": mr2_beta, "lb_weight": lb_weight}
...
BEST_CONFIGS = {
    "HateMM":       {"mr2_alpha": 0.3, "mr2_beta": 0.0, "lb_weight": 0.01, "seed": 159042},
    "MHC_En":       {"mr2_alpha": 0.1, "mr2_beta": 0.1, "lb_weight": 0.01, "seed": 9042},
    "MHC_Ch":       {"mr2_alpha": 0.1, "mr2_beta": 0.0, "lb_weight": 0.01, "seed": 15042},
    "ImpliHateVid": {"mr2_alpha": 0.3, "mr2_beta": 0.0, "lb_weight": 0.01, "seed": 2042},
}
...
def main():
    ...
    parser.add_argument("--mr2_alpha", type=float, default=None)
    parser.add_argument("--mr2_beta", type=float, default=None)
    parser.add_argument("--lb_weight", type=float, default=None)
    ...
    # Use best config if not overridden
    cfg = BEST_CONFIGS[tag]
    mr2_alpha = args.mr2_alpha if args.mr2_alpha is not None else cfg["mr2_alpha"]
    mr2_beta = args.mr2_beta if args.mr2_beta is not None else cfg["mr2_beta"]
    lb_weight = args.lb_weight if args.lb_weight is not None else cfg["lb_weight"]
    seed_offset = args.seed_offset if args.seed_offset is not None else (cfg["seed"] - 42)

    logger = setup_logger(f"MR2_{tag}")
    nc = 2; base_mk = ["text", "audio", "frame"]
    logger.info(f"=== SCM Q-MoE+QELS+MR2: {tag}, Runs: {args.num_runs}, "
                f"mr2_alpha={mr2_alpha}, mr2_beta={mr2_beta}, lb_weight={lb_weight} ===")
    ...
    save_dir = f"./seed_search_scm_qmoe_qels_mr2/{tag}_a{mr2_alpha}_b{mr2_beta}_lb{lb_weight}_off{seed_offset}"

    best = run(feats, splits, lm, base_mk, nc, args.num_runs, seed_offset,
               class_weight=[1.0, 1.5], save_dir=save_dir, logger=logger,
               eps_min=args.eps_min, eps_lambda=args.eps_lambda,
               mr2_alpha=mr2_alpha, mr2_beta=mr2_beta, lb_weight=lb_weight)

    logger.info(f"\n{'='*60}")
    logger.info(f"  MR2 {tag} BEST: ACC={best['metrics']['acc']:.4f} F1={best['metrics']['f1']:.4f}")
    logger.info(f"{'='*60}")
