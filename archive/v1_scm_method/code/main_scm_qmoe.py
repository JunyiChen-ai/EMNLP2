"""
SCM + Quadrant Mixture-of-Experts (Q-MoE).

Replaces SPCA+kNN retrieval with 4 quadrant-specific expert classifiers.
Each expert specializes in one SCM prejudice pathway.
Gating from Quadrant Composer's softmax distribution.

Usage:
  python main_scm_qmoe.py --dataset_name HateMM --num_runs 200 --seed_offset 0
"""
...
class SCMQMoE(nn.Module):
    """SCM Fusion + Quadrant Mixture-of-Experts classifier.

    Instead of a monolithic head, 4 small expert MLPs each specialize in
    one SCM quadrant (contempt/envy/pity/admiration). The Quadrant Composer's
    softmax distribution serves as the gating function.
    """
    def __init__(self, base_mk, hidden=192, nh=4, nc=2, drop=0.15, md=0.15,
        ...
        # Shared pre-classifier (same as original SCM)
        cd = nh * hidden + hidden + hidden + hidden + 1  # base + quadrant + percept + harm
        ...
        # Q-MoE: 4 quadrant-specific experts
        # Each expert: small MLP taking shared features → binary logits
    ...
    def forward(self, batch, training=False, return_penult=False):
        ...
        quadrant_dist = torch.softmax(quadrant_logits, dim=-1)  # [B, 4] — gate weights
        ...
        shared = self.pre_cls(fused)  # [B, expert_hidden]

        # 7. Q-MoE: route through 4 experts using quadrant_dist as gates
        expert_outputs = torch.stack([expert(shared) for expert in self.experts], dim=1)  # [B, 4, nc]
        logits = (expert_outputs * quadrant_dist.unsqueeze(-1)).sum(dim=1)  # [B, nc]

        if return_penult:
            return logits, shared
        return logits

    def load_balance_loss(self, batch):
        """Compute load balancing loss to prevent expert collapse."""
        # Re-compute quadrant dist (could cache, but cheap enough)
        field_h = {}
        for f in SCM_FIELDS:
            field_h[f] = self.field_projs[f](batch[f"scm_{f}"])
        base_feats = [p(batch[k]) for p, k in zip(self.base_projs, self.base_mk)]
        base_ctx = torch.stack(base_feats, dim=1).mean(dim=1)
        warmth_input = torch.cat([field_h["warmth_evidence"], field_h["target_group"], base_ctx], dim=-1)
        comp_input = torch.cat([field_h["competence_evidence"], field_h["target_group"], base_ctx], dim=-1)
        warmth_repr = self.warmth_stream(warmth_input)
        comp_repr = self.competence_stream(comp_input)
        wc_cat = torch.cat([warmth_repr, comp_repr], dim=-1)
        quadrant_dist = torch.softmax(self.quadrant_head(wc_cat), dim=-1)
        # Load balance: encourage uniform usage across experts
        avg_gate = quadrant_dist.mean(dim=0)  # [4]
        lb_loss = self.n_quadrants * (avg_gate * avg_gate).sum()  # CV-squared proxy
        return self.load_balance_weight * lb_loss


# ---- Utils (same as main_scm.py) ----
def cw(opt, ws, ts):
    def f(s):
...
def setup_logger(tag=""):
    ...
    lf = f"./logs/scm_qmoe_{tag}_{ts}.log"
    logger = logging.getLogger(f"scm_qmoe_{tag}_{ts}")
...
def run(feats, splits, lm, base_mk, nc, num_runs, seed_offset, class_weight, save_dir, logger,
        expert_hidden=64, load_balance_weight=0.01):
    ...
    for ri in range(num_runs):
        ...
        model = SCMQMoE(base_mk, nc=nc, expert_hidden=expert_hidden,
        ...
        for e in range(ep):
            ...
            for batch in trl:
                ...
                lb_loss = model.load_balance_loss(batch)
        ...
        # Still do retrieval sweep on penultimate features for fair comparison
        ...
        if (ri + 1) % 20 == 0:
            ...
            haccs = [r["head_acc"] for r in all_results]
            logger.info(f"  [{ri+1}/{num_runs}] mean={np.mean(accs):.4f} max={np.max(accs):.4f} "
                        f"head_mean={np.mean(haccs):.4f} head_max={np.max(haccs):.4f}")

    accs = [r["best_acc"] for r in all_results]
    haccs = [r["head_acc"] for r in all_results]
    logger.info(f"  FINAL: mean={np.mean(accs):.4f}+/-{np.std(accs):.4f} max={np.max(accs):.4f}")
    logger.info(f"  HEAD:  mean={np.mean(haccs):.4f}+/-{np.std(haccs):.4f} max={np.max(haccs):.4f}")
...
def main():
    ...
    parser.add_argument("--expert_hidden", type=int, default=64)
    ...
    logger = setup_logger(f"QMoE_{tag}")
    nc = 2
    base_mk = ["text", "audio", "frame"]

    logger.info(f"=== SCM Q-MoE: {tag}, Runs: {args.num_runs}, Offset: {args.seed_offset} ===")
    logger.info(f"  expert_hidden={args.expert_hidden}, load_balance={args.load_balance_weight}")
    ...
    save_dir = f"./seed_search_scm_qmoe/{tag}_off{args.seed_offset}"

    best = run(feats, splits, lm, base_mk, nc, args.num_runs, args.seed_offset,
               class_weight=[1.0, 1.5], save_dir=save_dir, logger=logger,
               expert_hidden=args.expert_hidden, load_balance_weight=args.load_balance_weight)

    logger.info(f"\n{'='*60}")
    logger.info(f"  SCM Q-MoE {tag} BEST RESULT")
