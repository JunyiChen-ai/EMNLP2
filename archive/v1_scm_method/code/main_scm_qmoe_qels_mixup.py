"""
SCM + Q-MoE + QELS + W-C Manifold Mixup.

Round 004: Add mixup in warmth-competence space to improve coverage
on small datasets. SCM says W-C is continuous → interpolation is theory-consistent.

Usage:
  python main_scm_qmoe_qels_mixup.py --dataset_name HateMM --num_runs 200 --seed_offset 0
"""
...
class SCMQMoEQELSMixup(nn.Module):
    def __init__(self, base_mk, hidden=192, nh=4, nc=2, drop=0.15, md=0.15,
        ...
        self.n_quadrants = n_quadrants; self.mixup_alpha = mixup_alpha
    ...
    def _encode_base_and_fields(self, batch, training):
        base_feats = []
        ...
        return base_repr, warmth_repr, comp_repr, percept_repr

    def _head(self, base_repr, wc_cat, percept_repr):
        ...
        return logits, q_entropy_norm, shared

    def forward(self, batch, training=False, return_penult=False, return_mixup=False):
        base_repr, warmth_repr, comp_repr, percept_repr = self._encode_base_and_fields(batch, training)
        wc_cat = torch.cat([warmth_repr, comp_repr], dim=-1)

        if training and return_mixup and self.mixup_alpha > 0:
            # Manifold mixup in W-C space
            B = wc_cat.size(0)
            lam = np.random.beta(self.mixup_alpha, self.mixup_alpha)
            lam = max(lam, 1 - lam)  # ensure lam >= 0.5
            perm = torch.randperm(B, device=wc_cat.device)

            wc_mixed = lam * wc_cat + (1 - lam) * wc_cat[perm]
            base_mixed = lam * base_repr + (1 - lam) * base_repr[perm]
            percept_mixed = lam * percept_repr + (1 - lam) * percept_repr[perm]

            logits, q_entropy, _ = self._head(base_mixed, wc_mixed, percept_mixed)
            return logits, q_entropy, lam, perm

        logits, q_entropy, shared = self._head(base_repr, wc_cat, percept_repr)
        if return_penult:
...
def qels_mixup_cross_entropy(logits, labels_a, labels_b, lam, q_entropy, nc=2,
                             eps_min=0.01, eps_lambda=0.15, class_weight=None):
    """Mixup loss with QELS."""
    loss_a = qels_cross_entropy(logits, labels_a, q_entropy, nc, eps_min, eps_lambda, class_weight)
    loss_b = qels_cross_entropy(logits, labels_b, q_entropy, nc, eps_min, eps_lambda, class_weight)
...
def setup_logger(tag=""):
    ...
    lf = f"./logs/scm_qmoe_qels_mixup_{tag}_{ts}.log"
    logger = logging.getLogger(f"mixup_{tag}_{ts}")
...
def run(feats, splits, lm, base_mk, nc, num_runs, seed_offset, class_weight, save_dir, logger,
        expert_hidden=64, eps_min=0.01, eps_lambda=0.15, mixup_alpha=0.4):
    ...
    for ri in range(num_runs):
        ...
        model = SCMQMoEQELSMixup(base_mk, nc=nc, expert_hidden=expert_hidden,
                                  eps_min=eps_min, eps_lambda=eps_lambda,
                                  mixup_alpha=mixup_alpha).to(device)
        ...
        for e in range(ep):
            ...
            for batch in trl:
                ...
                logits, q_entropy, lam, perm = model(batch, training=True, return_mixup=True)
                loss = qels_mixup_cross_entropy(logits, batch["label"], batch["label"][perm], lam,
                                               q_entropy, nc=nc, eps_min=eps_min, eps_lambda=eps_lambda,
...
def main():
    ...
    logger = setup_logger(f"Mixup_{tag}")
    nc = 2; base_mk = ["text", "audio", "frame"]
    logger.info(f"=== Q-MoE+QELS+Mixup: {tag}, Runs: {args.num_runs}, alpha={args.mixup_alpha} ===")
    ...
    save_dir = f"./seed_search_scm_qmoe_qels_mixup/{tag}_off{args.seed_offset}_a{args.mixup_alpha}"

    best = run(feats, splits, lm, base_mk, nc, args.num_runs, args.seed_offset,
               class_weight=[1.0, 1.5], save_dir=save_dir, logger=logger,
               mixup_alpha=args.mixup_alpha, eps_min=args.eps_min, eps_lambda=args.eps_lambda)

    logger.info(f"\n{'='*60}")
    logger.info(f"  Mixup {tag} BEST: ACC={best['metrics']['acc']:.4f} F1={best['metrics']['f1']:.4f}")
    logger.info(f"{'='*60}")
