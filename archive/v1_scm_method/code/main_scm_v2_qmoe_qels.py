"""
SCM + Q-MoE + Quadrant-Entropy Adaptive Label Smoothing (QELS).

Round 003: Replace fixed label smoothing with adaptive smoothing conditioned
on quadrant entropy. Ambiguous quadrant → softer supervision.

Usage:
  python main_scm_v2_qmoe_qels.py --dataset_name HateMM --num_runs 200 --seed_offset 0
"""
...
class SCMQMoEQELS(nn.Module):
    """SCM + Q-MoE + Quadrant-Entropy Adaptive Label Smoothing."""
    ...
    def forward(self, batch, training=False, return_penult=False, return_qels=False):
        ...
        field_h = {}
        for f in SCM_FIELDS:
            h = self.field_projs[f](batch[f"scm_v2_{f}"])
            if training and self.md > 0:
                h = h * (torch.rand(h.size(0), 1, device=h.device) > self.md).float()
            field_h[f] = h
        ...
        percept_input = torch.cat([field_h["social_perception"], field_h["endorsement_context"], field_h["behavioral_tendency"]], dim=-1)
        ...
        if return_qels:
            # Compute per-sample quadrant entropy for adaptive label smoothing
            q_entropy = -(quadrant_dist * (quadrant_dist + 1e-8).log()).sum(dim=-1)  # [B]
            q_entropy_norm = q_entropy / np.log(self.n_quadrants)  # normalize to [0,1]
...
def setup_logger(tag=""):
    ...
    lf = f"./logs/scm_v2_qmoe_qels_{tag}_{ts}.log"
    logger = logging.getLogger(f"qels_{tag}_{ts}")
...
def run(feats, splits, lm, base_mk, nc, num_runs, seed_offset, class_weight, save_dir, logger,
    ...
    all_mk = list(base_mk) + [f"scm_v2_{f}" for f in SCM_FIELDS]
...
def main():
    ...
    logger = setup_logger(f"QELS_{tag}")
    nc = 2; base_mk = ["text", "audio", "frame"]
    logger.info(f"=== SCM SCMv2-QMoE-QELS: {tag}, Runs: {args.num_runs}, eps_min={args.eps_min}, eps_lambda={args.eps_lambda} ===")
    ...
    for f in SCM_FIELDS:
        feats[f"scm_v2_{f}"] = torch.load(f"{emb_dir}/scm_v2_{f}_features.pth", map_location="cpu")
    with open(ann_path) as f:
        feats["labels"] = {d["Video_ID"]: d for d in json.load(f)}

    splits = load_split_ids(split_dir)
    save_dir = f"./seed_search_scm_v2_qmoe_qels/{tag}_off{args.seed_offset}"
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"  QELS {tag} BEST: ACC={best['metrics']['acc']:.4f} F1={best['metrics']['f1']:.4f}")
    logger.info(f"{'='*60}")
