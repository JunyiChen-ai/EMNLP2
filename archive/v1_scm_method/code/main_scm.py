"""
Phase C: SCM (Stereotype Content Model + BIAS Map) Full Model.

Dual-stream (Warmth + Competence) → Quadrant Composer → Quadrant Attractor.
Per-field modalities: text + audio + frame + 5 SCM fields.

Usage:
  python main_scm.py --dataset_name HateMM --num_runs 200 --seed_offset 0
"""
...
class SCMFusion(nn.Module):
    def __init__(self, base_mk, hidden=192, nh=4, nc=2, drop=0.15, md=0.15, n_quadrants=4):
        super().__init__()
        ...
        # Classifier: base_routing + quadrant_repr + perception + harm_score
    ...
    def forward(self, batch, training=False, return_penult=False):
        ...
        quadrant_logits = self.quadrant_head(wc_cat)  # [B, 4]
        quadrant_dist = torch.softmax(quadrant_logits, dim=-1)  # [B, 4]

        # Quadrant-aware representation
        quadrant_repr = torch.mm(quadrant_dist, self.quadrant_protos)  # [B, hidden]

        # 6. Harm score from quadrant distribution
        harm_score = (quadrant_dist * self.harm_weights.unsqueeze(0)).sum(dim=-1, keepdim=True)  # [B, 1]

        # 7. Perception features (social_perception + behavioral_tendency)
...
def setup_logger(tag=""):
    ...
    lf = f"./logs/scm_seed_search_{tag}_{ts}.log"
    logger = logging.getLogger(f"scm_{tag}_{ts}")
...
def run(feats, splits, lm, base_mk, nc, num_runs, seed_offset, class_weight, save_dir, logger):
    ...
    for ri in range(num_runs):
        ...
        model = SCMFusion(base_mk, nc=nc).to(device)
...
def main():
    ...
    logger = setup_logger(f"SCM_{tag}")
    nc = 2
    base_mk = ["text", "audio", "frame"]

    logger.info(f"=== SCM Full Model: {tag}, Runs: {args.num_runs}, Offset: {args.seed_offset} ===")
